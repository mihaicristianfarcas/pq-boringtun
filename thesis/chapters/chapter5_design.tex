...
\begin{description}[style=nextline, leftmargin=0pt]
    ...
    The initiator reads $\mathit{ct}$ from the responder's message and runs $\mathsf{ML\text{-}KEM.Decaps}(\mathit{dk}_i, \mathit{ct})$, recovering $\mathit{ss}_\mathit{kem}$. Both peers now hold the same $\mathit{ss}_\mathit{kem}$.
