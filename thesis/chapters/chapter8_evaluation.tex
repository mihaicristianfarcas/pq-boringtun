...
\midrule
Hybrid  & invalid  & 16{,}511 & 0.110 &  6.66 \\
Hybrid  & valid    & 16{,}515 & 0.370 & 22.40 \\
\bottomrule
\end{tabular}
\end{table}

The within-mode ratio is what confirms the short-circuit. For vanilla, the full-crypto path costs 4.8$\times$ more CPU than the reject path; for hybrid, 3.4$\times$ more. If MAC1 verification ran after Encaps rather than before, both ratios would sit close to 1 and the reject path would inherit the full crypto cost. They don't, so the kind of ``valid MAC1 doubles your attack surface'' argument that gets made about post-quantum handshake additions does not actually apply to this implementation. The hybrid build still pays more per rejected packet in absolute terms (6.66~$\mu$s versus vanilla's 3.09~$\mu$s, a 2.2$\times$ gap), but that comes from parsing the larger 1{,}332-byte initiation and from the rate-limit table lookup hashing more bytes, not from running any Kyber operation.

A practical reading of these numbers: at the worst measured rate (22.40~$\mu$s per packet, hybrid valid), the Pi 5 spends about 37~ms of CPU per second of wall time keeping up with a 16{,}500-pps flood, or roughly 3.7\% of one core. That is well within the headroom a responder needs to keep serving legitimate handshakes alongside the flood. The cookie-reply path, which kicks in when the rate limiter detects overload and forces incoming inits to carry a valid MAC2, was deliberately not exercised here so that the MAC1 short-circuit could be measured in isolation; the experiment characterises the first line of defence, not the second.
