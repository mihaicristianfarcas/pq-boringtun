...
\textbf{MTU management} is the most pressing operational concern. Network paths with an effective MTU below 1{,}332 bytes will fragment the hybrid initiation message. The MTU sweep in \S\ref{sec:mtu_sweep} confirms that BoringTun's hybrid handshake completes cleanly across MTUs from 1280 to 1500 when the path is fragmentation-tolerant: the kernel splits and reassembles the larger PQ datagrams transparently, with at most a few hundred microseconds of overhead. What the testbed could not test is the case operators actually worry about: a firewall, middlebox, or ISP that silently drops IP fragments. Linux netfilter's automatic conntrack defragmentation reassembles fragments before the filter table, so an iptables \texttt{-f -j DROP} rule does not bite; properly simulating a fragment-hostile middlebox requires either pre-conntrack hooks or a separate forwarding netns, both of which are future work. The recommended treatment is still to assume some paths drop fragments and to either negotiate a lower MTU for the VPN interface or push toward application-level segmentation or PMTUD-driven parameter selection, as the future-work list below describes.
