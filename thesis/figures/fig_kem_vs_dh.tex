...
\begin{tikzpicture}[
    ...
    % KEM Row 1: Responder generates keypair (asymmetric!)
    \coordinate (kem_r1b) at ($(kem_resp.south)-(0,2cm)$);
    \node[action, anchor=west] at ($(kem_r1b)+(0.3cm,0)$) {$(ek, dk) \gets \textsf{KeyGen}()$\\Public: $ek$, Private: $dk$};
    
    % KEM Row 2: Responder sends encapsulation key
    \coordinate (kem_m1_start) at ($(kem_resp.south)-(0,4.5cm)$);
    \coordinate (kem_m1_end) at ($(kem_init.south)-(0,4.5cm)$);
    ...
    % KEM Row 3: Initiator encapsulates
    \coordinate (kem_r3a) at ($(kem_init.south)-(0,6cm)$);
    \node[action, anchor=east] at ($(kem_r3a)+(-0.3cm,0)$) {$(ss, ct) \gets \textsf{Encaps}(ek)$\\Obtains shared secret $ss$};
    
    % KEM Row 4: Initiator sends ciphertext
    \coordinate (kem_m2_start) at ($(kem_init.south)-(0,9.5cm)$);
    \coordinate (kem_m2_end) at ($(kem_resp.south)-(0,9.5cm)$);
    ...
    % KEM Row 5: Responder decapsulates
    \coordinate (kem_r5b) at ($(kem_resp.south)-(0,12cm)$);
    \node[action, anchor=west] at ($(kem_r5b)+(0.3cm,0)$) {$ss \gets \textsf{Decaps}(dk, ct)$\\Obtains shared secret $ss$};
