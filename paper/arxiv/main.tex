...
The novelty here is one of engineering and evidence rather than cryptographic construction. The hybrid follows the well-understood concatenate-then-KDF combiner, and the contribution lies in realizing it inside a production codebase and in measuring its deployment cost across hardware tiers, network conditions, and adversarial load --- not in a new protocol. The security argument is correspondingly informal; a formal proof in the eCK-PFS-PSK model is left as future work.
