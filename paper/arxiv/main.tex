...
\author{
  ...
  Cluj-Napoca, Romania\\
  \texttt{wrap\_alphas.8s@icloud.com}
}
...
\textbf{State extension.} The \texttt{HandshakeState} structure is extended with fields for the ML-KEM ephemeral keypair. On the initiator side, an \texttt{EncapsulationKey768} and \texttt{DecapsulationKey768} are stored. On the responder side, the received encapsulation key and the encapsulated ciphertext are stored.
...
The PSK-slot baseline is implemented as a separate Rust CLI binary, \texttt{pq-psk-tool}, that operates independently of BoringTun. It uses the same \texttt{ml-kem} crate and provides three subcommands implementing a manual three-step ML-KEM-768 key exchange: \texttt{keygen} (generates an ML-KEM-768 keypair, encapsulation key as base64 for transfer, decapsulation seed kept secret), \texttt{encaps <ek\_file>} (runs ML-KEM.Encaps, outputs ciphertext as base64 and shared secret as hex PSK), and \texttt{decaps <dk\_file> <ct\_file>} (runs ML-KEM.Decaps, recovers the identical 32-byte PSK). The encapsulation key and ciphertext files are exchanged out-of-band; the tool itself does not implement a network protocol. Both peers then configure their WireGuard interface with the derived PSK using \texttt{wg set <iface> peer <PUBKEY> preshared-key psk.hex}.

\subsection{Test harness}

The \texttt{noise::tests} module contains unit tests for both vanilla and PQ handshake paths, conditionally compiled. They cover PQ handshake message validation (initiation = 1332~B, response = 1180~B), full PQ handshakes (four-step exchange including keepalive), end-to-end data transport, simultaneous PQ + PSK operation, and verification that a vanilla build rejects PQ message types. A separate shell script automates operational testing on macOS with loopback tunnel, comparison, and demo modes.
...
\begin{table}[H]
\centering
\caption{Handshake packet sizes and post-handshake transport throughput (\texttt{iperf3}, $t=15$~s).}
...
Throughput is essentially identical across configurations, as expected. All three use the same ChaCha20-Poly1305 data plane once the handshake is done, so throughput parity is the only reasonable outcome; any difference would point to an implementation bug. The $\sim$490~Mbps M4 loopback figure is CPU-bound on a single core; the $\sim$78~Mbps Wi-Fi figure reflects the link, not the Pi's crypto capacity. A Pi-only loopback test is not included because the Linux kernel routes traffic between two WireGuard interfaces on the same host directly, bypassing encryption; without network namespaces to force real encapsulation the measurements would not be reliable.
...
It is worth being explicit about where this work sits relative to existing literature. It targets a production Rust codebase (BoringTun), unlike most academic proposals that use prototype implementations. It uses NIST-standardized ML-KEM (FIPS 203), unlike PQ-WireGuard (Saber, McEliece) and Rosenpass (Kyber before standardization). It is hybrid (X25519 + ML-KEM), unlike PQ-WireGuard which is purely post-quantum. It modifies the Noise handshake directly for the full hybrid, unlike the PSK-slot approaches (Rosenpass, ExpressVPN). And it evaluates across multiple hardware platforms including ARM, providing empirical data relevant to constrained deployments.

The closest academic work is Hybrid-WireGuard~\cite{lafourcade2025hybrid}, which provides formal security proofs for a hybrid protocol. This paper complements that work: where they provide the proofs, this paper provides an implementation in a real-world Rust codebase and empirical performance data across three platforms.
...
Source code for the modified BoringTun (with the \texttt{pq} Cargo feature), the \texttt{pq-psk-tool} binary, the Criterion benchmark harness, the network-namespace testbeds for MTU and RTT/loss sweeps, the handshake-flood DoS example, and the memory probe is available in the project repository.
